...
//
////=================================================
//// ЭКСПЕРТНЫЙ УРОВЕНЬ - УЛУЧШЕННАЯ БЛОКИРОВКА
////=================================================
//
...
//
//                        // Для бесконечного поля нет проверки границ
...
//                    long long emptyRow = -1, emptyCol = -1;
//
//                    // Простой подсчет фигур игрока подряд
//                    while (1) {
...
//    };
//
//    // Сначала ищем позиции, которые находятся МЕЖДУ двумя фигурами игрока
//    for (long long i = ctx->bbox.minx - 2; i <= ctx->bbox.maxx + 2; i++) {
//        for (long long j = ctx->bbox.miny - 2; j <= ctx->bbox.maxy + 2; j++) {
...
//            }
//
//            // Если нашли позицию между двумя фигурами игрока - это идеальный ход
//            if (playerPairs > 0) {
//                *bx = i;
//                *by = j;
//                return true;
...
//    }
//
//    // Если не нашли позиций между фигурами, ищем клетки с максимальным количеством соседей-игроков
...
//    };
//
//    // 1. Оцениваем блокировку ВСЕХ направлений от этой клетки
//    for (int dir = 0; dir < 8; dir++) {
//        int deltaRow = directions[dir][0];
//        int deltaCol = directions[dir][1];
//
//        // Проверяем последовательности игрока в этом направлении
...
//
//    // 3. Штрафуем за ходы в углах и на краях (если не блокируем)
...
//    }
//
//    // 4. Бонус за центральные позиции
...
//        // Для бесконечного поля бонус за близость к другим фигурам
//        long long distanceFromOrigin = llabs(row) + llabs(col);
//        score -= distanceFromOrigin * 2; // Штраф за удаление от центра игры
//    }
//
//    return score;
//}
//
//// Поиск лучшей позиции блокировки (упрощенный минимакс на 1 шаг)
//bool find_best_blocking_position(GameContext* ctx, long long* bx, long long* by) {
//    // 1. Сначала проверяем критические угрозы (игрок может выиграть следующим ходом)
...
//    }
//
//    // 2. Ищем лучшую позицию для блокировки среди всех возможных ходов
//    long long bestRow = -1, bestCol = -1;
//    int bestScore = INT_MIN;
//
//    // Проверяем область вокруг занятых клеток
//    for (long long i = ctx->bbox.minx - 2; i <= ctx->bbox.maxx + 2; i++) {
//        for (long long j = ctx->bbox.miny - 2; j <= ctx->bbox.maxy + 2; j++) {
//            // Проверяем границы для ограниченного поля
//            if (ctx->parameters.infinite_field == 0 &&
...
//
//            // Дополнительный бонус за ходы в центре
//            if (ctx->parameters.infinite_field == 0) {
//                long long center = ctx->parameters.size / 2;
...
//}
//
//// Главная функция экспертного уровня
//void expert_computer_move(GameContext* ctx) {
//    //printf("Компьютер использует экспертную логику...\n");
//
//    long long bx, by;
//
//    // 1. ?? КРИТИЧЕСКИЕ УГРОЗЫ - блокировать немедленно!
...
//    }
//
//    // 2. ?? УМНАЯ БЛОКИРОВКА с оценкой
//    if (find_best_blocking_position(ctx, &bx, &by)) {
//        //printf("Компьютер делает оптимальный блокирующий ход!\n");
//        insert(ctx->board, bx, by, ctx->parameters.ai);
//        bbox_on_place(&ctx->bbox, bx, by);
//        ctx->parameters.last_ai_x = bx;
//        ctx->parameters.last_ai_y = by;
...
//    }
//
//    // 3. ?? БЛОКИРОВКА ПОСЛЕДОВАТЕЛЬНОСТЕЙ
...
//    }
//
//    // 4. ?? АКТИВНАЯ БЛОКИРОВКА - ставить рядом с фигурами игрока
...
//    }
//
//    // 5. ?? Используем обычный минимакс как запасной вариант
//    //printf("Компьютер использует обычную логику как запасной вариант.\n");
...
//            if (ctx->parameters.difficulty == 1) ctx->parameters.depth = 0;
//            else if (ctx->parameters.difficulty == 2) ctx->parameters.depth = 4;
//            else if (ctx->parameters.difficulty == 3) ctx->parameters.depth = 6;
//            else ctx->parameters.depth = 8;
//        }
...
//    }
//
//    // Если уровень EXPERT (4), используем улучшенную логику
//    if (ctx->parameters.difficulty == 4) {
//        expert_computer_move(ctx);
//    }
//    else {
//        minimax_move(ctx->board, &ctx->parameters, &ctx->bbox, ctx);
//    }
